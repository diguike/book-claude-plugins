---
title: commit-commands：Git 工作流一键化
feishu_url: "https://fivwvysqdz.feishu.cn/wiki/W9aNwbmLaiVM4ykAMbZcP8itnvg"
last_synced: "2026-05-24T23:59:00+08:00"
---

# commit-commands：Git 工作流一键化

三个 [Claude Code](https://claude.com/claude-code) slash command 覆盖日常 git 操作：`/commit` 自动提交，`/commit-push-pr` 一步到 PR（Pull Request），`/clean_gone` 清理过期本地分支。
...
这个命令的设计很精炼。frontmatter（YAML 元数据头）里 `allowed-tools` 只开了三个权限：
...
`/commit-push-pr` 需要 GitHub CLI（Command Line Interface，命令行工具）：
