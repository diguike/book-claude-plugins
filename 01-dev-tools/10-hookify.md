---
title: hookify：用 Markdown 配置 Claude Code 的行为拦截规则
feishu_url: "https://fivwvysqdz.feishu.cn/wiki/BtAgwpSREiHfsIkNvxociZcmnOb"
last_synced: "2026-05-24T23:59:12+08:00"
---

# hookify：用 Markdown 配置 Claude Code 的行为拦截规则

一句话：hookify 让你用 Markdown 文件定义规则，在 [Claude Code](https://claude.com/claude-code) 执行工具之前/之后拦截特定操作，可以弹警告也可以直接阻止。

## 技术原理

Claude Code 有一套 Hook（钩子，挂载到 Claude Code 事件的脚本/prompt）机制，允许在工具执行的各个阶段插入自定义脚本。hookify 利用这套机制，注册了四个 Python 脚本分别挂载到四个事件：
...
| UserPromptSubmit | `userpromptsubmit.py` | 用户提交 prompt（提示词）时 |
...
`config_loader.py` 有一个手写的 YAML frontmatter（YAML 元数据头）解析器（没有用 PyYAML，零外部依赖）。它从项目根目录的 `.claude/` 文件夹扫描所有 `hookify.*.local.md` 文件，解析 frontmatter 得到规则定义，解析 markdown body 得到提示消息。
...
`rule_engine.py` 的 `RuleEngine` 类负责评估。它把 hook 输入的各字段（`command`、`file_path`、`new_text` 等）提取出来，跟规则的 conditions 逐个比较。支持 6 种运算符：`regex_match`、`contains`、`equals`、`not_contains`、`starts_with`、`ends_with`。正则用 `lru_cache` 缓存（cache）编译结果（最多 128 个）。
...
另外还有一个 `conversation-analyzer` Agent 和一个 `writing-rules` Skill。前者在你不带参数运行 `/hookify` 时启动，扫描对话历史寻找你纠正过 Claude 的地方，自动建议规则。后者是规则写法的参考手册。命令侧还有 `/hookify:list`、`/hookify:configure`、`/hookify:help` 三个辅助命令，分别用来查看、启用/禁用和查文档。
