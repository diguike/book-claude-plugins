---
title: LSP 集成概述与通用原理
feishu_url: "https://fivwvysqdz.feishu.cn/wiki/F6Prw8QV2iFYcCkcoxAcjwY2nGc"
last_synced: "2026-05-24T23:59:37+08:00"
---

# LSP 集成概述与通用原理

Claude Code 本身能读写文件、能搜索代码，但它缺一个关键能力：**静态分析**。它不知道一个函数被谁调用了，不知道某个类型的定义在哪，不知道你改了一行代码之后哪些地方会报错。这些事情，IDE（Integrated Development Environment，集成开发环境）里的语言服务器（Language Server）天天在干。

claude-plugins-official 仓库里一口气提供了 12 个 LSP（Language Server Protocol，语言服务器协议）插件，覆盖 TypeScript、Python、Go、Rust、C/C++、Java、Kotlin、C#、Ruby、PHP、Swift、Lua。它们的作用一样：把对应语言的语言服务器接入 Claude Code，让 Claude 在改代码之前能先做类型检查、跳转定义、查引用。

## LSP 协议是什么

LSP 是微软在 2016 年从 VS Code 里抽出来的一套协议。核心思路很简单：把语言的智能分析能力（补全、诊断、跳转、重构等）封装成一个独立进程，通过 JSON-RPC（JSON Remote Procedure Call，基于 JSON 的远程过程调用）跟编辑器通信。编辑器不需要自己实现每种语言的分析逻辑，语言服务器也不需要关心编辑器的 UI 怎么画。
...
2. **启动语言服务器进程**。在后台启动对应的 LSP 服务器（比如 `typescript-language-server`、`pyright`、`gopls`），通过 stdio（standard input/output，标准输入输出）建立 JSON-RPC 通信。
