---
title: security-guidance：编辑时的安全风险提醒
feishu_url: "https://fivwvysqdz.feishu.cn/wiki/FW3qwp2GaiZBQdksurfc4whFnjb"
last_synced: "2026-05-24T23:59:27+08:00"
---

# security-guidance：编辑时的安全风险提醒

一个 PreToolUse（工具调用前触发的 Hook 类型）插件，在 Claude 编辑文件时实时检测安全风险模式（命令注入、XSS（Cross-Site Scripting，跨站脚本）、不安全的反序列化等），弹出警告阻止操作。

## 技术原理

这个插件跟前面几个完全不同——它不用 Agent（subagent）、不用 command、不用 Skill，纯粹靠 **Hook** 机制工作。
...
**规则覆盖面窄**。8 条内容匹配规则加 1 条路径匹配规则（GitHub Actions 工作流），总共 9 条，只覆盖了一小部分常见安全问题。SQL injection（SQL 注入）、路径遍历、SSRF（Server-Side Request Forgery，服务端请求伪造）、不安全的加密用法、硬编码密钥——这些都不在检测范围内。OWASP（<https://owasp.org>）和 CWE（<https://cwe.mitre.org>）目录里列的高频漏洞类型，这个插件只能算覆盖了入门级。
